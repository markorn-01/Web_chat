...
let room = ""
const token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..u1md1Fo1YMtr3KN_vQbVfMg8h4BDeuAzbKQqyxNd1vM"
...
socket.on('user-chat', (message) => {
    ...
    if (userid === message.userid)
    {
        ...
        {
            createDivConversation(activeUsername, activeRoom, activeAvatar, "You: " + message.message)
...
socket.on('change-room', data => {
    ...
    if (img === null)
    {
        ...
        document.querySelector(".right").querySelector(".client").insertBefore(img, document.querySelector(".right").querySelector(".client").firstChild)
    }
...
fetch("https://tiemthuocgiadinh.click/employee?bid=2",    
 {headers: {
  ...
  .then(data => {
    users = data.map(user => {
      ...
      return { username: user.ename, userID: user.eid.toString(), userURL: user.url, element: card }
    })
    users.forEach(user => { 
      user.element.classList.toggle("hide", true);
    })
    ...
    for (var i = 0, len = userCards.length; i < len; i++) {
        ...
        {
            room = e.target.parentElement.querySelector("[data-body]").firstChild.nodeValue.toString()
            username = e.target.parentElement.querySelector("[data-header]").firstChild.nodeValue
            activeRoom = room
            activeUsername = username
...
function enableClickConversation()
{
    ...
    for (var i = 0, len = x.length; i < len; i++)
    {
        x[i].addEventListener("click", (e) => {
            ...
            else if (e.target.classList.contains("icon-container") || e.target.classList.contains("box"))
            {
            ...
            else 
            {
                ...
                username = e.target.parentElement.parentElement.querySelector(".conversationName").textContent
                activeRoom = room
                activeUsername = username
